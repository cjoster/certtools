...
Under the hood of most all of these file formats is what's called ASN.1.
Getting into the details of ASN.1 is beyond the scope of this workshop, but it's
mentioned for completeness and to introduce OpenSSL's `asn1parse` sub-command.
...
Incidentally, if you're running a new enough version of OpenSSL that outputs PKCS#8 private keys,
...
You can take that whole octet string, hex decode it back into binary, and re-parse it. Just remember that it's binary now and thus in DER format.
